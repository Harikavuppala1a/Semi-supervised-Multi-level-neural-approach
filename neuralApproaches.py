    ...
    if use_saved_model and os.path.isfile(fname_mod_op) and map_ind == len(filename_map_list)-1:
        ...
        if use_saved_model and os.path.isfile(fname_mod):
            ...
            if map_ind !=0 :
                ...
                model_source, model_multi_task_or_tr_source, att_mod_source, post_vec_source = get_model(model_type, data_dict['max_post_length'], data_dict['max_num_sent'], data_dict['max_words_sent'], word_feats, sent_enc_feats, learn_rate, dropO1, dropO2, num_cnn_filters, rnn_type, loss_func, nonlin, numclasses_maplist[map_ind-1], rnn_dim, att_dim, max_pool_k_val, stack_rnn_flag, cnn_kernel_set,st_variant,use_conf_scores,multi_task_tl,share_weights_sep_mt,single_inp_tasks_list,sep_inp_tasks_list,False,n_fine_tune_layers,tf_hub_path, output_representation, bert_trainable)
...
def transform_labels(data_trainY, prob_trans_type, test_mode, save_fold_path, filename, n_clusters,train_ratio, NUM_CLASSES,map_filename):
